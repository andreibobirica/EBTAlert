...
class Authentification
{
    ...
    /**
     * Function that validate an input parameter again SQLINJECTION problems or against security problems
     * @param $param
     * @return bool|string
     */
    function validate($param)
    {
        //Controll for sqlinjection validating string
        $chparam = mysqli_real_escape_string($this->db->getConn(), $param);
        //Control with vocabular items
        $control = true;
        ...
        for ($pos = 0; $pos < strlen($chparam); $pos++) {
            $car = substr($chparam, $pos, 1);
            if (strpos($admited, $car) === false) $control = false;
        }
